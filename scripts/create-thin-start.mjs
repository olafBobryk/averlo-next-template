...
const PARKED_REFERENCE_DIR = path.join(
	PARKED_ROOT,
	"reference/webvizion-components",
);
...
const LIVE_REWRITE_TARGETS = [
	...
	},
	{
		...
		purpose:
			"Replace current Webvizion toast with shadcn/Sonner-style thin-start default.",
	},
	{
	...
	},
	{
		...
		purpose: "Remove broad Webvizion helper surfaces from live source.",
	},
];
const LIVE_IMPORT_PATTERN =
	/((from|import)\s*["'][^"']*(\.thin-start|thin-start\/reference|reference\/webvizion-components)[^"']*["']|import\([^)]*["'][^"']*(\.thin-start|thin-start\/reference|reference\/webvizion-components)[^"']*["'][^)]*\))/;
...
async function assertTemplateShape() {
	...
	if (missingScripts.length > 0) {
		throw new Error(
			`Expected a Webvizion template-shaped checkout. Missing package script(s): ${missingScripts.join(", ")}.`,
		);
	}
}
...
function buildManifest(options) {
	return {
		...
		notes: [
			"Reference-only parked copy of the current Webvizion component system.",
...
function printPlan(options) {
	...
	console.log("- default Webvizion setup remains unchanged");
	if (options.force) {
