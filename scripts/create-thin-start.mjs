...
const LIVE_REWRITE_TARGETS = [
	...
	},
	{
		...
		purpose:
			"Replace current Verilo toast with shadcn/Sonner-style thin-start default.",
	},
	{
	...
	},
	{
		...
		purpose: "Remove broad Verilo helper surfaces from live source.",
	},
];
const LIVE_IMPORT_PATTERN =
	/((from|import)\s*["'][^"']*(\.thin-start|thin-start\/reference|reference\/verilo-components)[^"']*["']|import\([^)]*["'][^"']*(\.thin-start|thin-start\/reference|reference\/verilo-components)[^"']*["'][^)]*\))/;
...
async function assertTemplateShape() {
	...
	if (missingScripts.length > 0) {
		throw new Error(
			`Expected a Verilo template-shaped checkout. Missing package script(s): ${missingScripts.join(", ")}.`,
		);
	}
}
...
function buildManifest(options) {
	return {
		...
		notes: [
			"Reference-only parked copy of the current Verilo component system.",
...
function printPlan(options) {
	...
	console.log("- default Verilo setup remains unchanged");
	if (options.force) {
