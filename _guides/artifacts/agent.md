...
Attention records that the owning agent needs steward or human attention. It is
owned by the agent doc and projected through the agent marker. It is not a graph
node and it is not a dashboard command surface.

Use attention for lightweight review points, blockers, failed checks, or
informational notices that should be visible from the dashboard. Clearing or
removing the `## Attention` section closes the event.

Candidate states:

- `info`: appears in the attention list without a marker badge.
- `attention`: needs steward review or action.
- `blocked`: cannot continue until the target is resolved.
- `failed`: verification or execution failed and needs attention.

`Title:` and `Body:` are the preferred card content when attention exists.
`Summary:` is tolerated as an older fallback. `Targets:` are secondary links;
the owning agent remains the event identity.

```markdown
## Attention

State: attention
Title: Review branch split
Body: Needs review before the next branch opens.
Targets:
- Thread: `019...`
- Run: `runs/example.md`
- Node: `example-workpiece`
- Preview: `http://localhost:3000/example`
```
