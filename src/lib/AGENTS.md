...
3. Add or update a demo in `src/app/(site)/(internal)/demo/content.tsx` when the feature is reusable, interactive, or intended for agent discovery.
...
## Demo Guidance
- Demo project foundations through `src/app/(site)/(internal)/demo/content.tsx`, usually under the matching utility area such as `lib/api` or related foundations pages.
