...
- Add inverted safety for the canonical `webvizion-template` `main` checkout:
...
- Passed: simulated canonical `webvizion-template` `main` dry-run is allowed.
- Passed: simulated canonical `webvizion-template` `main` mutating prune
  hard-stops without `--confirm-template-root`.
- Passed: disposable renamed instance at `/tmp/webvizion-prune-reset.ACGxLF`
