...
| ARB-TOOL-001 | Averlo Rebrand | Deterministic scroll-performance harness with internal fixture page and benchmark scripts. | `src/app/(site)/(marketing)/internal/scroll-performance/page.tsx`<br>`scripts/measure-scroll-performance.mjs`<br>`docs/worklogs/scroll-performance-benchmark.md` | Internal tooling | Adapt | Implemented | Added Playwright-backed measurement tooling and a prunable internal route; product scenarios from source were skipped. | Complete for generic `control`, `default`, and `stress` scenarios; route belongs to `--no-scroll-performance`, not `--no-demo`. |
| ARB-TOOL-002 | Averlo Rebrand | Scroll-performance autoresearch loop. | `scripts/setup-scroll-performance-autoresearch.mjs`<br>`scripts/score-scroll-performance-candidate.mjs`<br>`scripts/_lib/scroll-performance-autoresearch.mjs` | Experimental workflow | Adapt | Implemented | Creates disposable worktrees, writes runtime state under ignored `tmp/`, and can hard-reset discarded candidates inside the tagged worktree. | Complete as part of the scroll-performance tooling surface; do not run scoring from a worktree with unrelated commits or uncommitted work. |
...
| ARB-CONTENT-002 | Averlo Rebrand | Static-first resolver and site-content boundary with small render props. | `src/lib/site-content/pages.ts`<br>`src/lib/marketing-content/resolvers.ts`<br>`src/lib/marketing-content/types.ts` | Content modes | Defer | Stale/deferred | Source shapes are product IA, assets, and copy heavy; the template already has a lightweight fallback/content-mode contract. | Revisit only with explicit generic multi-page template scope. |
...
| 2026-07-04 | Accept the full generic scroll-performance system as one prunable tooling surface. | `ARB-TOOL-001` and `ARB-TOOL-002` are separate layers of the same workflow: the internal route measures deterministic scroll behavior, while the autoresearch scripts create/score disposable candidate worktrees. The template keeps only generic scenarios and removes the whole system with `--no-scroll-performance`. | accepted |
...
| --- | --- | --- | --- |
| Scroll-performance tooling gate | `ARB-TOOL-001`, `ARB-TOOL-002` | Implemented as a generic, prunable internal tooling surface with measurement, recording, and disposable autoresearch scoring. | Dependency gate for Playwright; prune dry-run; ignored-output check; browser fixture check. |
...
| 2026-07-04 | `npm run lint`; `npm run typecheck`; `npm run build`; `npm run prune:template -- --dry-run --no-scroll-performance`; lightweight prune dry-run with `--no-scroll-performance`; `npm run prune:template -- --yes --no-scroll-performance`; `npm run measure:scroll-performance -- --scenario control --output tmp/scroll-performance-smoke.json --notes "verification smoke"`; `npm run record:scroll-performance -- --input tmp/scroll-performance-smoke.json --output tmp/scroll-performance-runs-smoke.jsonl --notes "verification smoke record"`; `curl -fsS http://localhost:3069/internal/scroll-performance?scenario=default\\&motion=off\\&reveal=off`; `git diff --check` | passed / expected failure | Verified the generic scroll-performance system builds, measures, records ignored runtime output, returns 200 on the existing agent preview, and is removable through a dedicated prune surface. The mutating prune command failed as expected on canonical `main` without `--confirm-template-root`. |
