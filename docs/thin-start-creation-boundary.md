...
path. It defines the decision boundary only; it does not implement a creation
path, backport code, or change the default Webvizion template setup.
...
the plan is accepted. In-place mode parks the current Webvizion reference first,
then applies the selected thin-start live rewrite: shared primitive templates,
Sonner-backed toast, route-owned internal intelligence components, and removal
of broad misc/helper/icon/demo/dashboard surfaces from the live source graph.
...
accepted in the thin-start consolidation discussion and is tracked in
